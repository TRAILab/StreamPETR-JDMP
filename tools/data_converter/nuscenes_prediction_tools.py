...
def get_forecasting_annotations(nusc: NuScenes, annotations, length):
    ...
    for annotation in annotations:
        ...
        visibility = True
        next_sample_token = annotation["sample_token"]
        for step in range(length):
            ...
            next_ann_token = annotation['next']
            if next_ann_token != '':
                next_annotation = nusc.get('sample_annotation', next_ann_token)
                next_sample_token = nusc.get('sample', sample_token)['next']
                if next_annotation['sample_token'] == next_sample_token:
                    annotation = next_annotation
                    visibility = True
                else:
                    # if the trajectory is temporarily occluded, 
                    # use the last one to pad and set the visibility flag
                    annotation = annotation
                    visibility = False
            else:
                # if the trajectory ended,
                # use the last one to pad and set the visibility flag
    ...
    return forecast_boxes, forecast_visibility_mask, forecast_trajectory_type
