# import torch
# from projects.mmdet3d_plugin.pgp_datasets.interface import SingleAgentDataset
# import numpy as np
...
# ray.init(num_cpus=num_cpus, log_to_driver=False, runtime_env={'env_vars': { 'GRPC_ENABLE_FORK_SUPPORT': 'True', 'GRPC_POLL_STRATEGY': 'epoll1', 'RAY_start_python_importer_thread': '0', }})
# ray.shutdown()
...
def cluster_and_rank(k: int, data: np.ndarray):
    """
    Combines the clustering and ranking steps.
    """
...
def cluster_traj(k: int, traj: torch.Tensor):
    ...
    # Cluster and rank
    cluster_ops = [cluster_and_rank(k, data_slice) for data_slice in data]
    cluster_lbls = [cluster_op['lbls'] for cluster_op in cluster_ops]
    cluster_counts = [cluster_op['counts'] for cluster_op in cluster_ops]
    cluster_ranks = [cluster_op['ranks'] for cluster_op in cluster_ops]
