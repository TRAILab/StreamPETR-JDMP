...
@DETECTORS.register_module()
class JDMPRepDetr3D(MVXTwoStageDetector):
    """JDMPRepDetr3D."""
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 freeze_layer=None):
        super(JDMPRepDetr3D, self).__init__(pts_voxel_layer, pts_voxel_encoder,
                             pts_middle_encoder, pts_fusion_layer,
                             img_backbone, pts_backbone, img_neck, pts_neck,
                             pts_bbox_head, img_roi_head, img_rpn_head,
                             train_cfg, test_cfg, pretrained)
    ...
    def obtain_history_memory(self,
        ...
        for i in range(T):
            ...
            loss = self.forward_pts_train(gt_bboxes_3d[i],
                                        gt_labels_3d[i], gt_bboxes[i],
                                        gt_labels[i], gt_forecasting_bboxes_3d[i], gt_forecasting_masks[i], img_metas[i], centers2d[i], depths[i], requires_grad=requires_grad,return_losses=return_losses,**data_t)
            if loss is not None:
                for key, value in loss.items():
                    losses['frame_'+str(i)+"_"+key] = value
        return losses
    ...
    def forward(self, return_loss=True, **data):
        ...
        """
        if return_loss:
            for key in ['gt_bboxes_3d', 'gt_labels_3d', 'gt_bboxes', 'gt_labels', 'centers2d', 'depths', 'img_metas', 'gt_forecasting_bboxes_3d', 'gt_forecasting_masks']:
                data[key] = list(zip(*data[key]))
            return self.forward_train(**data)
        else:
            return self.forward_test(**data)
    ...
    def forward_train(self,
        ...
        losses = self.obtain_history_memory(gt_bboxes_3d,
                        gt_labels_3d, gt_bboxes,
                        gt_labels, gt_forecasting_bboxes_3d, gt_forecasting_masks, img_metas, centers2d, depths, gt_bboxes_ignore, **data)
